...
class GaussianMultinomialDiffusion(torch.nn.Module):
    def __init__(
        ...
        self._denoise_fn = denoise_fn  # 模型！！
    ...
    def mixed_loss(self, x, out_dict):
        ...
        x_in = torch.cat([x_num_t, log_x_cat_t], dim=1)  # 拼接连续特征和离散特征

        # # 在此处对数据结构进行变换成三维？
        # x_in_temp = copy.deepcopy(x_in)
        # x_in_3d = torch.from_numpy(x_in_temp.reshape((x_in_temp.shape[0], 1, x_in_temp.shape[1])))  # n, 1, m
        # x_in_3d = x_in_3d.float()
    ...
    @torch.no_grad()
    def sample(self, num_samples, y_dist, min_class):
        b = num_samples
        device = self.log_alpha.device
        z_norm = torch.randn((b, self.num_numerical_features), device=device)
    ...
    def sample_all(self, num_samples, batch_size, y_dist, min_class, ddim=False):
        if ddim:
        ...
        while num_generated < num_samples:
            sample, out_dict = sample_fn(b, y_dist, min_class)
